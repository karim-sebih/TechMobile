...
// Search Bar Toggle (exécuté après le chargement du DOM avec délégation)
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('Éléments trouvés :', { searchBtn, searchBar, searchInput, searchSubmit });

    if (searchBtn && searchBar && searchInput && searchSubmit) {
        // Délégation d'événements pour le bouton de recherche
        ...
        console.error('Un ou plusieurs éléments de la barre de recherche sont manquants :', {
            searchBtn,
            searchBar,
            searchInput,
            searchSubmit
...
function updateCart() {
    ...
    if (cart.length === 0) {
        ...
        cart.forEach(item => {
            ...
            cartItemElement.innerHTML = `
                <div class="w-20 h-20 bg-gray-100 rounded-lg flex items-center justify-center p-2">
                    <img src="${item.image || 'https://via.placeholder.com/150'}" alt="${item.name}" class="max-h-full max-w-full">
                </div>
                <div class="ml-4 flex-grow">
                    <h4 class="font-medium">${item.name}</h4>
                    <p class="text-gray-600">$${item.price.toFixed(2)}</p>
                    <div class="flex items-center mt-2">
                        <button class="change-quantity px-2 border rounded" data-id="${item.id}" data-change="-1">
        ...
        // Ajouter des écouteurs pour les boutons dynamiques
