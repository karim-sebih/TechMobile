    ...
    try {
        const response = await fetch(`${ROUTER_PATH}?resource=${encodeURIComponent(resource)}`, {
            ...
            headers: {
                ...
                'Accept': 'application/json' // Ajout pour demander JSON
            },
        });
        if (!response.ok) {
        ...
        let data;
        try {
            data = JSON.parse(text); // Tente de parser comme JSON
            ...
            console.warn('Réponse non JSON, utilisation brute:', text.substring(0, 100) + '...');
            return text; // Si ce n'est pas du JSON, utilise le texte brut (HTML)
        }
    } catch (err) {
