...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!MAIN_CONTAINER) console.error('MAIN_CONTAINER non trouvé');
});
...
async function getApiContent(resource) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return `<p>Erreur : ${response.statusText}</p>`;
        }
...
async function loadPageScript(filepath, args = []) {
    const importPath = `../pages/${filepath}`;
    console.log('Tentative d\'import de:', importPath);
    try {
        const script = await import(importPath);
...
function clearMainContainer() {
    if (MAIN_CONTAINER) MAIN_CONTAINER.innerHTML = '';
}
...
export async function loadPage(page, args = []) {
    ...
    if (MAIN_CONTAINER) MAIN_CONTAINER.innerHTML = content;
