...
async function loadPageStyle(filename, filepath) {
    if (await fileExists(filepath) && !linkAlreadyExists(filename)) {
        document.head.appendChild(createCSSLink(filename, `/Public/css/${filename}.css`)); // Ajuster le chemin si nécessaire
    }
}

async function loadPageScript(filepath, args = []) {
    const importPath = `/Public/js/pages/${filepath}`; // Ajuster le chemin si nécessaire
