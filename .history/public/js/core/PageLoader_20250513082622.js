...
async function loadPage(resource) {
    const MAIN_CONTAINER = document.getElementById('BodyLine');
    if (!MAIN_CONTAINER) {
        console.error('Élément <main id="BodyLine"> non trouvé dans le DOM');
        return;
    }
    try {
        const content = await getApiContent(resource);
        console.log('Contenu reçu du serveur:', content);
        MAIN_CONTAINER.innerHTML = content;
    } catch (error) {
        console.error('Erreur lors du chargement de la page:', error);
        MAIN_CONTAINER.innerHTML = `<p>Erreur : ${error.message}</p>`;
    }
}

// Gardez cette partie pour gérer la navigation initiale
document.addEventListener('DOMContentLoaded', () => {
    const initialPage = new URLSearchParams(window.location.search).get('resource') || 'home';
    loadPage(initialPage);
});