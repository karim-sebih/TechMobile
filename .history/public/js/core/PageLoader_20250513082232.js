...
async function loadPageScript(filepath) {
    try {
        const scriptModule = await import(`/js/pages/${filepath}`);
        if (scriptModule.init) scriptModule.init();
    } catch (err) {
        if (err.message.includes('404')) {
            console.warn(`Le script ${filepath} n'existe pas, ignoré`);
        } else {
            console.error(`Erreur lors de l'import de ${filepath}:`, err);
        }
    }
