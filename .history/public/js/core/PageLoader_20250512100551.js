...
async function loadPageScript(filepath, args = []) {
    try {
        const script = await import(`/js/pages/${filepath}`); // Use absolute path from project root
...
export async function loadPage(page, args = []) {
    ...
    await loadPageScript(`/Public/js/pages${page}.js`, args); // Chemin corrigé pour home.js
}
