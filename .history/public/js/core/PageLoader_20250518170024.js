...
async function displayPageHTML(html) {
    ...
    // 🔧 Correction automatique : éviter les <select> imbriqués
    MAIN_CONTAINER.querySelectorAll('select select').forEach(innerSelect => {
        console.warn('Sélecteur imbriqué supprimé automatiquement.');
        innerSelect.remove();
    });
}
