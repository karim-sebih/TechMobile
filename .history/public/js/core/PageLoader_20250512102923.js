...
async function loadPageScript(filepath, args = []) {
    try {
        const script = await import(`../pages/${filepath}`); // Chemin corrigé
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const { MAIN_ID } = await import('./config.js');
...
class PageLoader {
    ...
    static async loadPage(searchParams) {
        ...
        try {
            ...
            this.mainContainer.innerHTML = '<p>Erreur lors du chargement de la page.</p>';
        }
    }
}
