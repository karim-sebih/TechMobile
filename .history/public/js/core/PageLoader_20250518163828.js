...
async function submitForm(form) {
    ...
    console.log('Soumission du formulaire pour:', resource, 'action:', action, 'data:', Object.fromEntries(formData));
    ...
    try {
        ...
        if (data.success) {
            ...
            // Rediriger ou recharger la page admin après succès
            await loadPage('admin');
            return `<p>${data.success}</p>`;
        }

        // Si la réponse contient du contenu (par exemple, le formulaire), l'afficher
        if (data.content) {
            return data.content;
        }
