...
// Vérifie si un fichier existe via une requête HEAD
async function fileExists(filepath) {
    try {
        const response = await fetch(filepath, { method: 'HEAD' });
        console.log(`Vérification de l\'existence de ${filepath} : ${response.ok}`);
        return response.ok;
    } catch (error) {
...
// Récupère le contenu via l'API (router.php)
async function getApiContent(resource) {
    const url = `${ROUTER_PATH}?resource=${encodeURIComponent(resource)}`;
    console.log('Appel à getApiContent avec resource:', resource);
    ...
    try {
        ...
        console.error('Erreur dans getApiContent:', err.message);
        return `<p>Erreur lors du chargement : ${err.message}</p>`;
    }
}

// Charge le fichier CSS de la page
async function loadPageStyle(filename) {
    const filepath = `/TechMobile/Public/css/${filename}.css`;
    console.log('Tentative de chargement du CSS:', filepath);
    if (await fileExists(filepath) && !linkAlreadyExists(filename)) {
        ...
        console.warn(`Fichier CSS ${filepath} non trouvé ou déjà chargé, ignoré`);
    }
}

// Charge le script JS de la page
async function loadPageScript(filepath, args = []) {
    const importPath = `/TechMobile/Public/js/pages/${filepath}`;
...
// Supprime les anciens liens CSS
...
// Vérifie si un lien CSS existe déjà
...
// Charge une page (contenu, style, script)
