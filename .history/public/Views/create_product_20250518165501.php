    ...
    try {
        $stmt = $db->prepare("
            INSERT INTO products 
                (product_name, image_url, brand_id, category_id, sku, price, is_active, created_at, updated_at) 
            VALUES 
                (:product_name, :image_url, :brand_id, :category_id, :sku, :price, :is_active, NOW(), NOW())
        ");
        $stmt->execute([
...
// Sécurisation des options
$brandOptions = '';
foreach ($brands as $brand) {
    $id = htmlspecialchars($brand['brand_id'], ENT_QUOTES, 'UTF-8');
    $name = htmlspecialchars(strip_tags($brand['brand_name']), ENT_QUOTES, 'UTF-8');
    $brandOptions .= "<option value=\"$id\">$name</option>";
}

$categoryOptions = '';
foreach ($categories as $category) {
    $id = htmlspecialchars($category['category_id'], ENT_QUOTES, 'UTF-8');
    $name = htmlspecialchars(strip_tags($category['category_name']), ENT_QUOTES, 'UTF-8');
