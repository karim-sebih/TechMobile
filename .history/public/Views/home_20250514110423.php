...
try {
    ...
    if (empty($products)) {
        ...
        $stmt = $db->prepare("SELECT p.*, pi.image_url AS product_image_url 
                             FROM products p 
    ...
    <div class="products-list">
        ...
        if (!empty($products)) {
            foreach ($products as $product) {
                // Utiliser l'image de product_images (product_image_url) si disponible, sinon image par défaut
                $imageUrl = !empty($product['product_image_url']) ? htmlspecialchars($product['product_image_url']) : 'https://via.placeholder.com/150';
