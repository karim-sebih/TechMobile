...
<section class="products">
   ...
   <?php
     $select_products = $conn->prepare("SELECT * FROM `products` WHERE is_active = 1"); 
     $select_products->execute();
     if($select_products->rowCount() > 0){
      while($fetch_product = $select_products->fetch(PDO::FETCH_ASSOC)){
         $image_url = $fetch_product['image_url'] ?? 'images/default_product.jpg';
   ?>
   <form action="" method="post" class="box">
      ...
      <input type="hidden" name="image" value="<?= htmlspecialchars($image_url); ?>">
      <button class="fas fa-heart" type="submit" name="add_to_wishlist"></button>
      <a href="quick_view.php?pid=<?= $fetch_product['product_id']; ?>" class="fas fa-eye"></a>
      <img src="<?= htmlspecialchars($image_url); ?>" alt="<?= htmlspecialchars($fetch_product['product_name']); ?>">
