...
try {
    ...
    error_log("home.php - Connexion à la base de données réussie à " . date('Y-m-d H:i:s'));
    // Vérifier si la table products existe et contient des données
    $stmt = $db->query("SHOW TABLES LIKE 'products'");
    if ($stmt->rowCount() > 0) {
        error_log("home.php - Table 'products' existe.");
        $stmt = $db->query("SELECT COUNT(*) FROM products WHERE is_active = 1");
        $count = $stmt->fetchColumn();
        error_log("home.php - Nombre de produits actifs : $count");
    } else {
        error_log("home.php - Table 'products' introuvable.");
    }
} catch (Exception $e) {
    error_log("home.php - Échec de la connexion à la base : " . $e->getMessage());
...
try {
    $db = \Database::getInstance();
    $stmt = $db->prepare("SELECT * FROM products WHERE is_active = 1 ORDER BY created_at DESC LIMIT 4");
    $stmt->execute();
    $products = $stmt->fetchAll(PDO::FETCH_ASSOC);
    error_log("home.php - Produits récupérés directement : " . json_encode($products));
} catch (Exception $e) {
    ...
    <div class="products-list">
        ...
        if (!empty($products)) {
            foreach ($products as $product) {
                ...
                echo '<img src="' . $imageUrl . '" alt="' . htmlspecialchars($product['product_name']) . '" class="product-image"><br>';
                echo '<h3>' . htmlspecialchars($product['product_name']) . '</h3>';
                echo '<p>Prix : ' . htmlspecialchars($product['price']) . ' €</p><br>';
                echo '<button class="add-to-cart" data-id="' . $product['product_id'] . '" data-name="' . htmlspecialchars($product['product_name']) . '" data-price="' . $product['price'] . '" data-image="' . $imageUrl . '">Ajouter au panier</button>';
                echo '</div>';
            }
        } else {
            echo '<pre>Débogage : $products est vide ou null. Contenu : ' . htmlspecialchars(print_r($products, true)) . '</pre>';
