...
if (!file_exists($databasePath)) {
    error_log("admi.php - Erreur : database.php non trouvé à $databasePath");
...
try {
    ...
    error_log("admi.php - Connexion à la base de données réussie à " . date('Y-m-d H:i:s'));
} catch (Exception $e) {
    error_log("admi.php - Échec de la connexion à la base : " . $e->getMessage());
...
try {
    ...
    error_log("admi.php - Produits récupérés : " . json_encode($products));
} catch (Exception $e) {
    error_log("admi.php - Erreur lors de la récupération des produits : " . $e->getMessage());
    $products = [];
}
?>
